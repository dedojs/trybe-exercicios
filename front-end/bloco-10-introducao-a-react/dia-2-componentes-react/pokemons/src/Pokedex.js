...
class Pokedex extends React.Component {
  ...
  filtro(event) {
    ...
    const filtrados = pokes.map((element) => element.name)
    this.setState({
      index: 0,
      name: filtrados
            ...
            <section>
                {
                  ...
                  .map((poke) => <Pokemon key={pokemons.name} pokemons={poke} />)
                }
            </section>
            <div>
              {
                newList
                  .map((item, index) => {
                  return (
                    <button key={item[index]} className="button" onClick={this.filtro}>{item}</button>
                  )
                })
              }
            </div>
